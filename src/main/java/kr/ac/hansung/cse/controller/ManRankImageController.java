...
import kr.ac.hansung.cse.repo.ManRankImageRepository;

@RestController
@RequestMapping("/rankImage/man")
public class ManRankImageController {

	@Autowired
	FeedImageRepository feedImageRepository;
	@Autowired
	ManRankImageRepository rankImageRepository;

	@GetMapping
	public ResponseEntity<Map<String, List<ManRankImage>>> getManRankImages() {
		List<ManRankImage> images = new ArrayList<>();
		Map<String, List<ManRankImage>> map = new HashMap<>();
		rankImageRepository.findAll().forEach(images::add);
		Set<String> timeStamp = new HashSet<>();
		for (ManRankImage image : images) {
			timeStamp.add(image.getRankTimeStamp());
		}
		for (String ts : timeStamp) {
			List<ManRankImage> i = rankImageRepository.findByRankTimeStamp(ts);
			map.put(ts, i);
		}
	...
	@PutMapping
	public ResponseEntity<List<ManRankImage>> updateManRankImage(LocalDateTime nowTimeStamp) {
		...
		List<ManRankImage> rImages = new ArrayList<>();
